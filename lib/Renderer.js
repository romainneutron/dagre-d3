...
function addLabel(value, root, marginX, marginY) {
  ...
  if (label[0] === '<') {
    ...
    textLabelSvg = addTextLabel(label, labelSvg);
  }

  var bbox = root.node().getBBox(),
      width = 'width' in value ? Number(value.width) : bbox.width,
      height = 'height' in value ? Number(value.height) : bbox.height;
  ...
  rect
    ...
    .attr('y', -(height / 2 + marginY))
    .attr('width', width + 2 * marginX)
    .attr('height', height + 2 * marginY);

  if (textLabelSvg) {
    textLabelSvg.attr("x", width / 2);
    textLabelSvg.attr("y", height / 2);
  }
}
